...
$(function() {
  ...
  // Получаем контейнер с продуктами
  var $productsContainer = $('#products');

  // При изменении значения выбранной категории
  $categorySelect.on('change', function() {
    // Получаем выбранную категорию
    var category = $(this).val();

    // Если выбрана категория "Все категории", показываем все продукты
    if (category === 'all') {
      $productsContainer.find('.card').show();
    } else {
      // Иначе скрываем все продукты и показываем только те, которые относятся к выбранной категории
      $productsContainer.find('.card').hide();
      $productsContainer.find('.card[data-category="' + category + '"]').show();
    }
  });
});
...
var dataContainer = document.getElementById('cards');
	...
	xhr.onload = function() {
		if (xhr.status === 200) {
			...
			for (var i = 0; i < responseData.length; i++) {
				...
				newItem.setAttribute('data-category', responseData[i].type_plant);

				var cardTop = document.createElement('div');
				cardTop.className = 'card__top';
				cardTop.setAttribute('id', "card-" + responseData[i].name_plant);

				var imageLink = document.createElement('a');
				imageLink.className = 'card__image';
				imageLink.href = '/product/' + responseData[i].id;

				var image = document.createElement('img');
				image.src = "/media/" + responseData[i].img_plant;
				...
				titleLink.href = '/product/' + responseData[i].id;
        	...
        	const newCards = [...document.querySelectorAll('[id^="card-"]:not(.animated)')];
              gsap.from(newCards, {
                opacity: 0,
                y: 50,
                duration: 1,
